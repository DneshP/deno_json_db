import { assertEquals, assertObjectMatch, Database } from "../deps.js";
...
Deno.test("find a collection which does not exists", async () => {
  ...
  assertResponse(
    response,
    false,
    "There is no users collection available, Try creating one first.",
  );
  db.removeDB("testDB");
});
...
Deno.test("Insert a record into a collection", async () => {
  ...
  assertResponse(getInserted, true, [{
    id: 1,
    name: "yo",
    email: "its@d.com",
  }]);
...
Deno.test("Update a records in a collection", async () => {
  ...
  const response = await db.update("users", {
    $find: { name: "yo", email: "its@d.com" },
    $set: { new: "update@d.com" },
  });
...
Deno.test("Validate update object signature", async () => {
  ...
  const response = await db.update("muggles", { name: "test" }, {
    name: "newTest",
  });
  assertResponse(
    response,
