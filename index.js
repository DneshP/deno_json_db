 ...
 export class Database {
    ...
    _initDB() {
        ...
        if(error instanceof Deno.errors.NotFound) {
            const [create,initError] = this.tryCatch(Deno.mkdirSync, this.directory, { mode: 0o777 });
            if(initError) {
    ...
    async createDirectory(name, directoryOptions = {}) {
        ...
        if(error instanceof Deno.errors.NotFound){
            const [ data, error ] = await this.wrapTryCatch(Deno.mkdir, directory, directoryOptions);
            return error ? this.getResponse(false, error) : this.getResponse(true, 'Directory created');
        }
    }
        ...
        for (const [key,updateObject] of Object.entries(update)) {
            for (const updateKey in updateObject) {
                if (Object.hasOwnProperty.call(updateObject, updateKey)) {
                        if(updateKey === '_id'){
                           return this.getResponse(false, 'Cannot update _id it is system generated value');
                        }
    ...
    async find(name, filterParam = undefined, getByIndex = false) {
        let filterData;
        const [rawData, error] = await this.wrapTryCatch(Deno.readTextFile, this.collectionPath(name));
        if(error instanceof Deno.errors.NotFound) {
            return this.getResponse(false, `There is no ${name} collection available, Try creating one first.`)
        }
            ...
            filterData = data.filter((each, index) => {
                for (const filterKey in filterParam){
                        if (Object.hasOwnProperty.call(filterParam, filterKey)) {
                            ...
                            if(!each.hasOwnProperty(filterKey) || !(each[filterKey] === element)){
                                return false;
                            }
                        }
                    }
        ...
        return error ? this.getResponse(false, error) : this.getResponse(true, filterData);
    }
    ...
    async insert(name, data) {
        if (data._id) {
            return this.getResponse(false, 'Cannot set _id value it is system generated value');
        }
        let insertData = data;
        ...
        const [info, insertError] = await this.submit(file, this.encoder.encode(JSON.stringify(insertData)));
        return insertError ? this.getResponse(false, 'Error inserting data') : this.getResponse(true, 'Data inserted');
    }
